...
class DecSALayer(nn.Module):
    ...
    def forward(
        ...
        if encoder_out is not None:
            ...
            # attn_weights, attn_logits = attn[0], attn[1]
            attn_weights = attn[0]
...
class TransformerEncoder(FFTBlocks):
    def __init__(self, dict_size, hidden_size=256, num_layers=4, kernel_size=9, num_heads=2,
                 dropout=0.0):
        super().__init__(hidden_size, num_layers, kernel_size, num_heads=num_heads,
                         use_pos_embed=False, dropout=dropout)  # use_pos_embed_alpha for compatibility
        self.embed_tokens = Embedding(dict_size, hidden_size, 0)
        self.pre_net = ConvBlocks(
            self.hidden_size, self.hidden_size, [1] * 3, 1, layers_in_block=2)
        self.embed_scale = math.sqrt(hidden_size)
        self.padding_idx = 0
        self.embed_positions = SinusoidalPositionalEmbedding(
            hidden_size, self.padding_idx, init_size=DEFAULT_MAX_TARGET_POSITIONS,
    ...
    def forward(self, txt_tokens, attn_mask=None):
        ...
        """
        txt_padding_mask = txt_tokens.eq(self.padding_idx).data
        x = self.forward_embedding(txt_tokens)  # [B, T, H]
        if self.num_layers > 0:
            x = super(TransformerEncoder, self).forward(x, txt_padding_mask, attn_mask=attn_mask)
        return x
...
class TransformerDecoder(nn.Module):
    # The transformer decoder used in the masked prediction task for mel
    def __init__(self, hidden_size, num_layers, ffn_kernel_size=9, dropout=0.0,
                 num_heads=2, use_pos_embed=True, use_last_norm=True,
                 use_pos_embed_alpha=True):
        super().__init__()
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            TransformerDecoderLayer(self.hidden_size, self.dropout,
                                    kernel_size=ffn_kernel_size, num_heads=num_heads)
            for _ in range(self.num_layers)
        ])
        if self.use_last_norm:
    ...
    def forward(self, x, encoder_out, padding_mask=None, attn_mask=None, return_hiddens=False):
        """
        :param x: [B, T, C]
        :param padding_mask: [B, T]
        :return: [B, T, C] or [L, B, T, C]
        """
        encoder_padding_mask = encoder_out.abs().sum(-1).eq(0).data
        padding_mask = x.abs().sum(-1).eq(0).data if padding_mask is None else padding_mask
        nonpadding_mask_TB = 1 - padding_mask.transpose(0, 1).float()[:, :, None]  # [T, B, 1]
        if self.use_pos_embed:
            positions = self.pos_embed_alpha * self.embed_positions(x[..., 0])
        ...
        attns = []
        for layer in self.layers:
            x, attn_logits = layer(x, encoder_out=encoder_out, encoder_padding_mask=encoder_padding_mask, attn_mask=attn_mask)
            x = x * nonpadding_mask_TB
            attns.append(attn_logits)
            hiddens.append(x)
        # enc_dec_attn = torch.stack(attns, dim=1).mean(dim=1)
        enc_dec_attn = attns[0]
        if self.use_last_norm:
            x = self.layer_norm(x) * nonpadding_mask_TB
        if return_hiddens:
            x = torch.stack(hiddens, 0)  # [L, T, B, C]
...
class MelEncoder(nn.Module):
    def __init__(self, input_dim=80, hidden_dim=192):
