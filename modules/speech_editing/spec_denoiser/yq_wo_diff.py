...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, txt_tokens, time_mel_masks, mel2ph, spk_embed,
        ...
        if not infer:
            ...
            x = torch.zeros(shape, device=device)  # noise
            #for i in tqdm(reversed(range(0, t)), desc='ProDiff Teacher sample time step', total=t):
                #x = self.p_sample(x, torch.full((b,), t, device=device, dtype=torch.long), cond)  # x(mel), t, condition(phoneme)
            x = self.denoise_fn(x, torch.full((b,), t, device=device, dtype=torch.long), cond) * nonpadding
            x = x[:, 0].transpose(1, 2)
